  ...
  <DropdownContent ref="dropdownRef" class="mcsl-tree-select">
    <template #triggerer="{ toggle, opened }">
      <button
        ...
        :style="{
          '--mcsl-tree-select__color-light': getColorVar(new ColorData(color, 'light')),
          '--mcsl-tree-select__color': getColorVar(color),
        }"
        @click="toggle"
      >
        <span v-if="selectedLabel">{{ selectedLabel }}</span>
        <span v-else class="mcsl-tree-select__placeholder">{{ placeholder }}</span>
        <span
...
.mcsl-tree-select__trigger:focus {
  ...
  outline-width: 2px;
  border-color: color-mix(in srgb, var(--mcsl-tree-select__color) 48%, var(--mcsl-border-color-base));
}

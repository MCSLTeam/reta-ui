...
<template>
  <div class="mcsl-time-picker" :class="[`mcsl-size-${size}`]">
    <input
      v-model="model"
      ...
      :step="step"
      :style="{
        '--mcsl-time-picker__color-light': getColorVar(new ColorData(color, 'light')),
        '--mcsl-time-picker__color': getColorVar(color),
      }"
      type="time"
      @blur="
        $emit('blur', $event);
...
@each $size in utils.$sizes {
  .mcsl-size-#{$size}.mcsl-time-picker {
...
.mcsl-time-picker > input:hover {
  border-color: var(--mcsl-border-color-dark);
  background: color-mix(in srgb, var(--mcsl-bg-color-dark) 94%, transparent);
}

.mcsl-time-picker > input:focus {
  outline-color: var(--mcsl-time-picker__color-light);
  outline-width: 2px;
  border-color: color-mix(in srgb, var(--mcsl-time-picker__color) 48%, var(--mcsl-border-color-base));
}

.mcsl-time-picker > input:disabled {
  cursor: not-allowed;
  opacity: 0.58;
}

.mcsl-time-picker > input[aria-invalid="true"] {

...
<template>
  <div class="mcsl-date-picker" :class="[`mcsl-size-${size}`]">
    <input
      ...
      :style="{
        '--mcsl-date-picker__color-light': getColorVar(new ColorData(color, 'light')),
...
.mcsl-date-picker > input:focus {
  ...
  outline-width: 2px;
  border-color: color-mix(in srgb, var(--mcsl-date-picker__color) 48%, var(--mcsl-border-color-base));
}

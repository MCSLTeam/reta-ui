...
<template>
  <div
    ...
    :style="{
      ...
      '--mcsl-calendar__color-soft': new ColorData(color, 'default', 0.12).getCss(),
    }"
  >
    <div class="mcsl-calendar__header">
      <Button icon="fa fa-angle-left" rounded size="small" type="text" @click="moveMonth(-1)" />
      <strong>{{ title }}</strong>
      <Button icon="fa fa-angle-right" rounded size="small" type="text" @click="moveMonth(1)" />
    </div>
...
.mcsl-calendar__day--today {
  border-color: color-mix(in srgb, var(--mcsl-calendar__color) 38%, transparent);
}
